...
| Objetos profundos, inventarios, recursos, transformaciones | §15 | S7 | `parcial` (modelo profundo de objetos/contenedores/recursos, gramática recoger/reparar/desmontar y 2 de 4 demostradores —armario/estantería, frigorífico— con generador y comportamiento real; faltan bomba de agua, carretilla/carro, `store`/`retrieve_from_storage`, deterioro de alimento fresco, catálogo completo de 14 familias, integración PostgreSQL y E2E) | `packages/contracts/src/objects-v2.ts`, `packages/catalogs/src/{action-methods,repair-disassembly-profiles,object-catalog}.ts`, `packages/simulation-core/src/v2/jobs/advance-jobs.ts`, `packages/simulation-core/src/v2/generator/buildings.ts` |
