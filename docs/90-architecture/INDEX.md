...
| [ARC-005](ARC-005_semantic-world-data-model.md) | `approved` (entidades conceptuales) | Modelo conceptual de datos del mundo semántico (`Building`, `Room`, etc.) y, desde `DESIGN-008`, del entorno mutable, accesos y transporte (`TerrainArea`, `LinearFeature`, `Opening`, `InstalledClosure`, `TransportMeans`, etc.). |
...
- `docs/decisions/` (`DEC-0001` a `DEC-0013` respaldan estas reglas;
  ...
  `DEC-0013` respalda la ampliación de `ARC-005` con las entidades del
  entorno mutable de `DESIGN-008`).
...
- `40-settlement` (modelo de objeto, familias logísticas y desmontaje que
  consume el motor de resolución; `SET-010`/`SET-011` aplican el mismo
  motor a transporte y agricultura).
- `20-world` (`WLD-010`/`WLD-011` definen el entorno mutable y los
  accesos cuyas entidades conceptuales amplía `ARC-005`).
