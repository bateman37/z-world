...
export function MapCanvas({
  ...
  const draw = useCallback(() => {
    ...
    for (const structure of mapEntities.structures) {
      ctx.beginPath();
    ...
    for (const point of mapEntities.points) {
    ...
    for (const person of mapEntities.people) {
      ...
      ctx.fillStyle = isSelected ? "#6fc79a" : "#e7ebee";
      ctx.fill();
      if (isSelected) {
        ctx.lineWidth = 2;
  ...
  function handleWheel(event: React.WheelEvent<HTMLCanvasElement>) {
    ...
    const nextPixelsPerMeter = Math.min(
    ...
    <div ref={containerRef} style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {contextAction && (
        <div
          ...
          style={{
            position: "absolute",
            left: contextAction.screenX + 8,
            top: contextAction.screenY + 8,
            padding: 6,
            zIndex: 10,
          }}
        >
...
function findNearestPerson(
  people: MapEntitiesProjection["people"],
